...
class Director(object, metaclass=ABCMeta):
    ...
    def get_contructed_object(self):
        return self._builder.construct_object


class AbstractFormBuilder(object, metaclass=ABCMeta):
    def __init__(self, ):
        self.construct_object = None
...
class HtmlBuilder(AbstractFormBuilder):
    def __init__(self):
        self.construct_object = HtmlForm()

    def add_text_field(self, field_dict):
        self.construct_object.field_list.append(
    ...
    def add_checkbox(self, checkbox_dict):
        ...
        self.construct_object.field_list.append(
            '<label><input type="checkbox" id={0} value={1}> {2}<br>'.format(
                checkbox_dict['field_id'], checkbox_dict['value'],
                checkbox_dict['label']))
...
if __name__ == '__main__':
    ...
    with open("form_file.html", "w") as f:
        f.write("<html><body>{0!r}</body></html>".format(
            director.get_contructed_object()))
