...
# 连接是一个阻塞操作，因此在返回之后调用setblocking()方法
server_addresss = ('localhost', 10000)
print('connecting to {} port {}'.format(*server_addresss))
sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
sock.connect(server_addresss)
...
while keep_running:
    ...
    for key, mask in mysel.select(timeout=1):
        ...
        print('clinet({})'.format(client_address))
    ...
    if mask & selectors.EVENT_WRITE:
        ...
        if not outgoing:
            # 我们的消息为空了，这意味着我们再也不必要写数据，所及将我们的配置更改为从服务器读取
